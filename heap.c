...
newHEAP(
    ...
    void (*f)(void *)) {
        ...
        assert(tree!=0);
        tree->bstree = newBST(d, compareINTEGER, NULL, f);
...
extractHEAP(HEAP *h) {
    ...
    temp = swapToLeafBST(h->bstree, temp);
    pruneLeafBST(h->bstree, temp);
    //displayBST(h->bstree, stdout);
    BSTNODE *newTemp = getBSTroot(h->bstree);
    //rebuild(h, newTemp);
    heapify(h, newTemp);
    setBSTsize(h->bstree, sizeBST(h->bstree)-1);
    h->size --;
